...
class TerminalBufferTest {
        ...
        fun `should initialize with empty content`() {
            for (row in 0 until buffer.height) {
                for (col in 0 until buffer.width) {
                    val cell = buffer.getCellChar(Coord(row, col))
                    assertEquals(' ', cell.char) // Default should be space
                }
            }
        }
    }
    ...
    inner class WriteTextComprehensiveTests {
        ...
        fun `should override existing content completely`() {
            ...
            assertEquals("New", line.substring(0, 3))
            // Characters after "New" should be spaces (cleared)
            assertEquals(' ', line[3])
            assertEquals(' ', line[4])
        }
        ...
        fun `should handle text exactly at line boundary`() {
            val exactText = "1234567890" // Exactly 10 characters for width 10
            buffer.writeText(exactText)
            assertEquals(Coord(0, 10), buffer.cursor) // Should be at end, potentially wrapping
        ...
        fun `should handle text overflow at line end`() {
            buffer.moveCursorRight(8) // Position at column 8
            buffer.writeText("LongText") // 8 characters, only 2 fit
            
            val line = buffer.getLine(0)
            assertEquals("Lo", line.substring(8, 10)) // Only first 2 chars fit
            // Cursor should be at end of line or wrap to next line
            assertTrue(buffer.cursor.column == 0 && buffer.cursor.row == 1 || 
                      buffer.cursor.column == 10 && buffer.cursor.row == 0)
        }
    ...
    inner class InsertTextComprehensiveTests {
        ...
        fun `should handle insertion at line end`() {
            ...
            assertEquals("Hello World", buffer.getLine(0).trim())
            assertEquals(Coord(0, 11), buffer.cursor)
        }
        ...
        fun `should handle insertion with newlines`() {
            ...
            assertEquals("AX", buffer.getLine(0).trim())
            assertEquals("YB", buffer.getLine(1).trim())
        }
    ...
    inner class ContentAccessComprehensiveTests {
        ...
        fun `should get correct cell character at all positions`() {
            ...
            for (row in 0 until buffer.height) {
                for (col in 0 until buffer.width) {
                    ...
                    val cell = buffer.getCellChar(Coord(row, col))
                    assertEquals(expectedChar, cell.char)
                }
            }
        }
        ...
        fun `should get correct attributes at all positions`() {
            for (row in 0 until buffer.height) {
                for (col in 0 until buffer.width) {
                    val attrs = buffer.getCellAttributes(Coord(row, col))
                    assertNotNull(attrs)
        ...
        fun `should handle out-of-bounds coordinates gracefully`() {
            val outOfBounds = listOf(
                ...
                Coord(buffer.height, 0), Coord(0, buffer.width),
                Coord(buffer.height, buffer.width),
                Coord(1000, 1000), Coord(-1000, -1000)
            )
            
            outOfBounds.forEach { coord ->
                assertDoesNotThrow { 
                    val cell = buffer.getCellChar(coord)
                    assertNotNull(cell)
                }
                assertDoesNotThrow {
                    val attrs = buffer.getCellAttributes(coord)
                    assertNotNull(attrs)
                }
            }
        }
        ...
        @Test
        @DisplayName("Should handle invalid line indices")
        fun `should handle invalid line indices`() {
        ...
        fun `should get all content including scrollback`() {
            ...
            // All content should include screen content
            val screenContent = buffer.getScreenContent()
            if (screenContent.isNotEmpty()) {
                assertTrue(allContent.contains(screenContent))
            }
        }
    }
    ...
    inner class EdgeCasesTests {
        ...
        fun `should handle operations at exact screen boundaries`() {
            ...
            corners.forEachIndexed { index, corner ->
                ...
                val cell = buffer.getCellChar(corner)
                assertEquals('C', cell.char)
        ...
        fun `should handle rapid successive operations`() {
            repeat(1000) { i ->
                val op = i % 4
                when (op) {
                    0 -> buffer.writeText("W$i")
                    1 -> buffer.insertText("I$i")
                    2 -> buffer.fillLine(('A' + i % 26).toChar())
                    3 -> {
        ...
        fun `should maintain buffer integrity after complex operations`() {
            ...
            for (row in 0 until buffer.height) {
                ...
                for (col in 0 until buffer.width) {
                    assertDoesNotThrow { buffer.getCellChar(Coord(row, col)) }
                    assertDoesNotThrow { buffer.getCellAttributes(Coord(row, col)) }
                }
            }
        ...
        fun `should handle operations on completely empty buffer`() {
            ...
            // Content access should return consistent empty results
            assertEquals("", emptyBuffer.getScreenContent())
            assertEquals("", emptyBuffer.getAllContent())
    ...
    inner class IntegrationTests {
        ...
        fun `should handle line wrapping edge cases`() {
            ...
            assertEquals(buffer.width, buffer.cursor.column)
